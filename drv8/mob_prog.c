/*
* Supported, but not recommended in CthulhuMud
*/
...
char *mprog_next_command( char *clist )
{
 ...
 /* If we get passed a NULL, we give it right back...               Mik Fix */         
  if (clist == NULL) {                                           /* Mik Fix */
    return clist;                                                /* Mik Fix */
  }                                                              /* Mik Fix */
 ...
 * If there are errors, then return 0 otherwise return boolean 1,0
 */

/* Mikfix. Compiler doesn't like returning -1 in a bool. It can't tell
the difference between -1 and 1. So I've changed all of the 'return -1's
into 'return 0's. Now lets see if the segmentation faults have gone away. */ 
...
//      switch ( arg[1] )  /* arg should be "$*" so just get the letter */
//	{
//	case 'i': if (mob->pcdata == NULL)
//				return 0; 
//			  lhsvl = mob->pcdata->clan_num;
//	          rhsvl = atoi( val );
//                return mprog_veval( lhsvl, opr, rhsvl );
//	case 'n': if ( actor )
//	          {
//			if (actor->pcdata == NULL)
//				return 0;
//		    lhsvl = actor->pcdata->clan_num;
//		    rhsvl = atoi( val );
//		    return mprog_veval( lhsvl, opr, rhsvl );
//		  }
//	          else 
  ...
  if ( !str_cmp( buf, "class" ) )
    {
      switch ( arg[1] )  /* arg should be "$*" so just get the letter */
	{
	case 'i': lhsvl = mob->pc_class;
	          rhsvl = atoi( val );
                  return mprog_veval( lhsvl, opr, rhsvl );
	case 'n': if ( actor )
	          {
		    lhsvl = actor->pc_class;
		    rhsvl = atoi( val );
		    return mprog_veval( lhsvl, opr, rhsvl );
		  }
	          else 
		   ...
		   OBJ_DATA *obj, void *vo)
{
 ...
 while ( cmnd != NULL && *cmnd != '\0' )                         /* Mik Fix */
   {
     morebuf = one_argument( cmnd, buf );
     if ( !str_cmp( buf, "if" ) )
    
      /* If this finds a break in the mob_prog, it will return      Mik Fix */
      /* a NULL, not a pointer to an empty string...                Mik Fix */      
...
/* Add a flag to stop each program triggering more than once...    Mik Fix */

  bool        matched = FALSE;                                  /* Mik Fix */ 

/* Only degunk the argument once, not once for each prog...        Mik Fix */

  strcpy( temp2, arg );
  dupl = temp2;
                                                                
/* Lose punctuation and other gunk...                              Mik Fix */

  for ( i = 0; i < strlen( dupl ); i++ )
    if (isalnum(dupl[i])) {                                     /* Mik Fix */
      dupl[i] = tolower(dupl[i]);                               /* Mik Fix */
    } else {                                                    /* Mik Fix */
      dupl[i] = ' ';                                            /* Mik Fix */
  }                                                             /* Mik Fix */

  for ( mprg = mob->pIndexData->mobprogs; mprg != NULL; mprg = mprg->next )
    if ( mprg != NULL )                                         /* Mik Fix */
    if ( mprg->type & type )
      {
        matched = FALSE;
	...
	if ( ( list[0] == 'p' ) && ( list[1] == ' ' ) )
	  {
	    list += 2;                                          /* Mik Fix */
	    while ( ( start = strstr( dupl, list ) ) && !matched )
...
/*		      || *end == NULL ) ) */
	  ...
	  {
	    list = one_argument( list, word );                  /* Mik Fix */
	    for( ; word[0] != '\0' && !matched; list = one_argument( list, word ) )
	      while ( ( start = strstr( dupl, word ) ) && !matched )
		if ( ( start == dupl || *(start-1) == ' ' )
...
/*  MPROG_ACT_LIST * tmp_act;                                        Mik Fix */

  if ( IS_NPC( mob )
      && ( mob->pIndexData->progtypes & ACT_PROG ) ) {

/* Uh-hm. Someone forgot to put a call in to make it do anything!    Mik Fix */
/* Again, mobs cannot drive off of their own actions...              Mik Fix */

    if (ch != mob) {                                              /* Mik Fix */ 
      mprog_wordlist_check( buf, mob, ch, NULL, NULL, ACT_PROG ); /* Mik Fix */
    }                                                             /* Mik Fix */ 
 
/*      tmp_act = alloc_mem( sizeof( MPROG_ACT_LIST ) );             Mik Fix
      if ( mob->mpactnum > 0 )
...
void mprog_death_trigger( CHAR_DATA *mob, bool corpse )
{
